...
class checkpoint():
    ...
    def write_result(self,content):
         open_type = 'a' if os.path.exists(self.get_path('example.txt'))else 'w'
         with open(self.get_path('example.txt'), open_type) as file:  
                file.write('\n'+content)
    ...
    def save_results(self, name, filename, save_list, scale):
        if self.args.save_results:
            ...
            for v, p in zip(save_list, postfix):
                ...
                self.queue.put(('{}.png'.format(filename), v))
