...
from .base import BaseDeepakeDetectionModel, Batch

# Create a simple logger fallback if not available
class SimpleLogger:
    def print(self, msg):
        print(msg)
...
class GenD(BaseDeepakeDetectionModel):
    def __init__(self, config: Config, verbose: bool = False):
        ...
        self.config = config
        # Save hyperparameters only if config has model_dump (Pydantic v2)
        if hasattr(config, 'model_dump'):
            self.save_hyperparameters(config.model_dump())
        self.is_debug_mode = "tmp" in config.run_name if hasattr(config, 'run_name') else False

        if verbose and hasattr(config, '__str__'):
            logger.print(config) if hasattr(logger, 'print') else print(config)
