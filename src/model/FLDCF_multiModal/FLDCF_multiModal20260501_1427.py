...
class Embeddings(nn.Module):
    ...
    """
    def __init__(self, config, img_size, in_channels=3, disable_hybrid=False):
        super(Embeddings, self).__init__()
        self.hybrid = None
        self.config = config
        self.disable_hybrid = disable_hybrid
        img_size = _pair(img_size)

        if config.patches.get("grid") is not None and not disable_hybrid:   # ResNet
        ...
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       ...
                                       stride=patch_size)
        self.patch_embeddingsd = Conv2d(in_channels=1,
                                       out_channels=config.hidden_size,
                                       kernel_size=patch_size,
                                       stride=patch_size)
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis, disable_hybrid=False):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(config, img_size=img_size, disable_hybrid=disable_hybrid)
    ...
    #     # score1, y, attn_weights, features, score3, size_conv1 = self.transformer(x, y) 
    ...
    def forward(self, input_ids, dsm_ids):
        # 跳过完整的 embeddings 流程，直接进入 encoder
        # 但需要先将原始图像转换为 encoder 期望的 [B, n_patch, hidden] 格式
        
        # 使用 patch_embeddings 卷积层处理输入
        x = self.embeddings.patch_embeddings(input_ids)
        y = self.embeddings.patch_embeddingsd(dsm_ids)
        
        # 展平并转置为 [B, n_patches, hidden]
        x = x.flatten(2).transpose(-1, -2)
        y = y.flatten(2).transpose(-1, -2)
        
        # 添加位置编码和 dropout
        embeddingsx = x + self.embeddings.position_embeddings
        embeddingsx = self.embeddings.dropout(embeddingsx)
        embeddingsy = y + self.embeddings.position_embeddings
        embeddingsy = self.embeddings.dropout(embeddingsy)
...
class FLDCF_multiModal(nn.Module): #FLDCF_multiModal
    def __init__(self, args,config, block=Bottleneck, layers=[1, 2, 2, 1], num_classes=2, aux=True, \
        ...
        self.transformer = Transformer(config, img_size, vis, disable_hybrid=True)
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            res_weight = weights
            # Load patch_embeddings weights only if input channels match
            embedding_kernel = np2th(weights["embedding/kernel"], conv=True)
            if embedding_kernel.size(1) == self.transformer.embeddings.patch_embeddings.weight.size(1):
                self.transformer.embeddings.patch_embeddings.weight.copy_(embedding_kernel)
                self.transformer.embeddings.patch_embeddings.bias.copy_(np2th(weights["embedding/bias"]))
            
            # For patch_embeddingsd (DSM branch), pre-trained weights have 3 input channels
            # but DSM input is single channel, so we average the weights across channels
            if embedding_kernel.size(1) == 3 and self.transformer.embeddings.patch_embeddingsd.weight.size(1) == 1:
                dsm_kernel = embedding_kernel.mean(dim=1, keepdim=True)  # Average across 3 channels to get 1 channel
                self.transformer.embeddings.patch_embeddingsd.weight.copy_(dsm_kernel)
